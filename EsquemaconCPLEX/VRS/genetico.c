...
//    struct timeval ti, tf;
    ...
    while(aux!=0.0&&maxiteraciones<((25*rellenar)+1))
    {
        ...
        if(metodo==0&&chrom->valido==0){
            ...
            aux+=check_constraint_VRS(chrom);
            if(aux==0.0)
            {
                chrom->valido=1;
                chrom->metodo = 1;
...
int check_constraint_VRS (Ptr_Chromosome c)
{
    int valido = 0,
    total = 0,
    i = 0;
...
void calculo_alfas (Ptr_Chromosome chrom, double *outputs, int col) {
    ...
    {
        for(i=0;i<CANTIDAD_S;i++){
            vector[i]=(chrom->beta*(outputs[i*CANTIDAD_N+col]-1.0))+chrom->tpos[i];
        }

        for(i=0;i<CANTIDAD_S;i++)
        {
            for(j=0;j<CANTIDAD_S;j++){
                matriz[(j*alfnocero)+i]=(outputs[(i*CANTIDAD_N)+indices[j]]-1.0);
    ...
    //system("pause");
        ...
        for(i=0;i<(alfnocero-CANTIDAD_S);i++){
            chrom->alfas[indices[i]]=((chrom->beta)/(CANTIDAD_S+1))-((Random())/5.0);
        }
        for(i=0;i<CANTIDAD_S;i++){
